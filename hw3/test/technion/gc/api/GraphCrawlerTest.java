 ...
 */
class TestC_BfsStrategy implements CrawlStrategy {

	Queue<TestCNode> q = new LinkedList<TestCNode>();
	Set<TestCNode> visited = new HashSet<TestCNode>();

	@Override
	public CrawlNode nextNode(CrawlNode nd) {
		if (nd instanceof TestCNode)
			visited.add((TestCNode) nd);
		if (null != nd.getNeighbors()) {
			for (CrawlNode n : nd.getNeighbors()) {
				if (n instanceof TestCNode)
					q.add(((TestCNode) n));
			}
		}
		CrawlNode next = null;
...
class TestC_DfsStrategy implements CrawlStrategy {
...
class TestCAction implements CrawlAction {

	public StringBuilder buff = new StringBuilder();

	@Override
	public void invoke(CrawlNode nd) {
		TestCNode actual = (TestCNode) nd;
		buff.append(actual.id);
	}
